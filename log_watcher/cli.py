...
@click.command()
@click.option('--file-path', default='/tmp/kern.log',
              help='File path to watch.', callback=validators.file_path)
@click.option('--regexp', default=r'match-this-string',
              help='Regular expression to match.', callback=validators.regexp)
@click.option('--period', default=30,
              help='How many minutes to wait between checks.')
def cli(file_path, regexp, period):
    ...
    tracker = CloudwatchTracker(name="%s_kernel_anomalies"
                                % get_hostname(), period=period)
